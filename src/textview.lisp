...
;;;;
;;;; This is the foundation for editors and other text widgets (the tvlisp
;;;; application builds its file editor and REPL on it).  Text is held as an
;;;; adjustable vector of line strings.  Editing, navigation and scrolling are
;;;; all driven through small primitives so that subclasses can override
;;;; behaviour -- in particular TEXT-RETURN, which decides what the Enter key
;;;; does.  Syntax highlighting is delegated to *TEXT-COLORIZER* / *PAREN-MATCHER*
;;;; (NIL here; an application installs language support).
...
;;; --- Syntax highlighting: a pluggable colouriser + bracket matcher ---------
;;; The framework itself knows no language.  When a view's TEXT-HIGHLIGHT slot is
;;; on, the flat draw path colours each line through *TEXT-COLORIZER* and accents
;;; matching brackets through *PAREN-MATCHER*, when an application installs them.
;;; (tvlisp installs Common Lisp implementations -- see its lisp-text.lisp; with
;;; neither installed, a highlighted view simply draws as plain text.)

(defun %synfg (base fg) (make-attr fg (attr-bg base)))

(defvar *text-colorizer* nil
  "When non-NIL, a function (LINE BASE-ATTR IN-STRING) -> (values ATTRS
END-IN-STRING).  ATTRS is a per-character attribute vector colouring LINE;
END-IN-STRING is whether LINE ends inside a multi-line string (threaded into the
next line's call).  Colours TEXT-HIGHLIGHT views; NIL means no colouring.")

(defvar *paren-matcher* nil
  "When non-NIL, a function (VIEW) -> a list of (LINE . COL) cells to accent as
matching brackets (e.g. the bracket at point and its partner), or NIL.")

(defun %colorizer-start-state (tv top)
  "Fold *TEXT-COLORIZER* over the lines above TOP for the in-string state line
TOP begins in (so a string opened earlier keeps colouring).  NIL with no
colouriser installed."
  (let ((instr nil))
    (when *text-colorizer*
      (dotimes (i (min top (line-count tv)))
        (setf instr (nth-value 1 (funcall *text-colorizer* (nth-line tv i) 0 instr)))))
    instr))
...
(defun %draw-flat (tv)
  (let* ((w (point-x (view-size tv)))
         ...
         (instr (and hl (%colorizer-start-state tv (text-top-line tv))))
         (parens (and hl (logtest (view-state tv) +sf-focused+)
                      *paren-matcher* (funcall *paren-matcher* tv)))
        ...
        (let ((li (+ (text-top-line tv) row)))
          (when (< li (line-count tv))
            ...
            (let* ((line (nth-line tv li)) (len (length line))
                   (start (min dx len))
                   (attrs (when (and hl *text-colorizer*)
                            (multiple-value-bind (a s) (funcall *text-colorizer* line c instr)
                              (setf instr s) a))))
